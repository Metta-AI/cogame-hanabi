// Hanabi static replay shell: fetches the replay named by ?replay=<url>,
// hands the bytes to the wasm module (which re-derives the frame timeline
...
(function () {
  ...
  // Frames to wait for the renderer's first drawn frame before giving up.
  var READY_FRAMES = 240;
  ...
  // actually drawn its first frame, `error` when the replay cannot be shown.
  // Same envelope shape as the ctf-shell Escape bridge ({src, type}); no
  ...
  // `ready` must mean A PICTURE, not a parsed payload: posting it two
  // animation frames after attachReplay can beat the renderer's first draw,
  // and the softmax.com embed then samples an unpainted shell (chorus
  // 3c11c953, 2026-08-24). Poll the attribute the renderer sets on its first
  // drawn frame instead, bounded so a renderer that never draws reports an
  // error rather than hanging silently.
  function tellReadyWhenDrawn() {
    var frames = 0;
    (function poll() {
  ...
  function start(module, bytes) {
    ...
    var ok = module._hb_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._hb_error_ptr(),
        module._hb_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._hb_payload_ptr(),
        module._hb_payload_len())
    );
    var loading = document.getElementById("loading");
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = HanabiReplayModule().catch(function (error) {

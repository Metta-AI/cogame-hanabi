...
# Substitute: hanabi, coworld-hanabi, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-hanabi:ci)
#   SMOKE_SLUG                 game slug                        (hanabi)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/hanabi)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/hanabi-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-hanabi:ci}}"
slug="${SMOKE_SLUG:-hanabi}"
...
# Every PLAYER container must have exited 0 too. A player killed by the game's
# own shutdown (whisky raises on a close frame, mummy's send only queues) exits
# 1 through no fault of its own, and the hosted worker reports that as a player
# failure — invisible here unless it is asserted (cogmud, 2026-08-24).
for ((slot = 0; slot < seats; slot++)); do
  deadline=$((SECONDS + 60))

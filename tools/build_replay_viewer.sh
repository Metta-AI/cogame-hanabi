...
# Create the PARENT before any containment check: `coworld build` passes a
# path inside a directory that may not exist yet, and resolving it first
# fails on a path that is perfectly legal (ecos, 2026-08-23).
...
test -s "${dist}/hanabi_replay.wasm"
test -s "${dist}/hanabi_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/hanabi_replay.js" "${dist}/hanabi_replay.wasm" "${output_dir}/"
...
echo "hanabi replay viewer bundle: ${output_dir}"
